...
/* UDP control */
enum udp_state udp_get_state() {
// In UDP_GLB_STAT register there is not diff between default state and power state.
	switch(UDP->UDP_GLB_STAT & 0x0003) {
		case UDP_GLB_STAT_FADDEN :	return UDP_STATE_ADDRESS;
		case UDP_GLB_STAT_CONFG  :	return UDP_STATE_CONFIGURE;
		default :					return _udp.state; // case 0x00
	}
}
